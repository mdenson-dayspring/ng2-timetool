...
describe('RC-4 PRNG Generator', function () {
    it('Can construct from string', function () {
        let sp = new RC4Gen('password');
        expect(sp.nextByte()).toBe(255);
    });
    it('Can construct from bytearray', function () {
...
describe('Spritz PRNG Generator', function () {
    it('Can construct from string', function () {
...
describe('LCG PRNG Generator', function () {
    it('Can construct from seed', function () {
        let sp = new LCGGen(0);
        expect(sp.nextByte()).toBe(135);
    });
    it('Can construct without seed', function () {
        let sp = new LCGGen();
    });
    it('Test Vectors', function () {
...
describe('Plugable PRNG', function () {
    it('Can construct with Spritz', function () {
        let rnd = new Rand(new SpritzGen('password'));
        expect(rnd.nextByte()).toBe(146);
    });
    it('nextByte with and withour modulo', function () {
        let rnd = new Rand(new SpritzGen('password'));
        expect(rnd.nextByte()).toBe(146);
        rnd = new Rand(new SpritzGen('password'));
        expect(rnd.nextByte(16)).toBe(2);
    });
    it('nextInt with and withour modulo', function () {
        let rnd = new Rand(new SpritzGen('password'));
